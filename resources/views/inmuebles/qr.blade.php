...
<head>
    ...
    <title>QR de Bienes Inmuebles</title>
    <style>
        /* Estilos para el PDF */
        body {
...
<body>
    ...
    <div class="page">
        <div class="row">
            @foreach($bienes_inmuebles as $index => $bienes_inmuebles)
                <div class="bienes_inmuebles-container">
                    <img class="qr-code" src="{{ $bienes_inmuebles->qr }}" alt="Código QR">
                    <div class="cve-inventario">
                        <p>NUM_ESCRITURA_PROPIEDAD: {{ $bienes_inmuebles->num_escritura_propiedad }}</p>
                    </div>
                </div>
                <?php $contador++; ?>
                @if ($contador % 3 == 0)
